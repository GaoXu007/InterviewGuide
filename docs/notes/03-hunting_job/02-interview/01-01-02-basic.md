...
- 考虑类的情况
  ...
  - const成员函数：由于const成员函数保证不更改成员变量，再加上const对象要求成员变量不能改动，因此得出：
    - const对象只能调用const成员函数（不更改成员变量），不能调用非const成员函数（可能更改成员变量）；
    - 非const对象（成员变量允许改动）能调用const成员函数（不更改成员变量），也能调用非const成员函数（可能更改成员变量）；
    - 另外，在const成员函数中可以修改用mutable关键字声明的变量的值；
...
**在C语言的头文件中，对其外部函数只能指定为extern类型，C语言中不支持extern "C"声明，在.c文件中包含了extern "C"时会出现编译错误。**
